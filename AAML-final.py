    ...
    return px.scatter(df, x=df[x], y=df[y], title="", animation_frame="Pressure",
                      ...
                      hover_data={}, template="none",
                                     ...
                                     mirror=True).update_yaxes(spikedash='solid',
                                                               showgrid=False, title=y, autorange=True, ticks='outside',
    ...
    return px.scatter(df, x=df[x], y=df[y], title="", animation_frame="Pressure",
                      ...
                      # color_continuous_scale='Viridis',
                                     ...
                                     mirror=True).update_yaxes(spikedash='solid',
                                                               ...
                                                               showline=True, mirror=True).update_layout(
        clickmode='event+select', hovermode='closest', margin={'l': 80}, autosize=True
    ).update_traces(marker=dict(sizemin=10,
                                opacity=0.7,
                                showscale=False,
                                line=dict(width=0.7, color='DarkSlateGrey'),
                                colorbar=dict(title=dict(text=color, side='right')),
                                colorscale="Viridis" ))
                            ...
                            figure={'data': [
                                go.Scatter(x=dff[xaxis_name], y=dff[yaxis_name],
                                           ...
                                           marker=dict(sizemode='area', sizeref=max(dff[marker_size]) / (15 ** 2),
                                                       ...
                                                       line=dict(width=0.7, color='DarkSlateGrey'),
                                                       colorbar=dict(title=dict(text=marker_color, side='right')),
...
def update_graph_stat(yaxis_name, percentile_type, data_set, contents, filename):
    ...
    for pressure in pressure_list:
        traces.append(go.Violin(y=data[data['Pressure'] == pressure][yaxis_name], name=pressure,
    ...
    return {'data': traces
        ...
        , 'layout': go.Layout(
            title=f"<b> Pressure against {''.join(str(i) for i in yaxis_name)} "
            ,
            xaxis=dict(rangeslider=dict(visible=True), title='Pressure'),
